...
public class HomePageTest {
    ...
    private void fillInFormSamsung() {
        ...
        onView(withId(R.id.tag_edit_text)).perform(ViewActions.typeText("mytag1, mytag2, mytag3"));
    ...
    private void fillInFormApple() {
        ...
        onView(withText("OK")).perform(click());
        onView(withId(R.id.comment_edit_text)).perform(ViewActions.typeText("my comment wbwbwbwbwbwb!!!"));
        onView(withId(R.id.tag_edit_text)).perform(ViewActions.typeText("mytag1, mytag2"));
        onView(withId(R.id.serial_number_edit_text)).perform(ViewActions.typeText("1234567890"));
    }

    // US 01.01.01 : As an owner, I want to add an item to my items, with a
    // date of purchase or acquisition, brief description, make, model,
    // serial number (if applicable), estimated value, and comment.

    // Following stories are loosely covered in this test:
