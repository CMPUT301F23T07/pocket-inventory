...
import java.util.Calendar;
    ...
    public class CameraTest {
        ...
        private void fillInapple(){
            ...
            com.example.pocketinventory.AddTest.setDate(R.id.date_of_purchase_edit_text, 2023, 9, 15);
            Espresso.onView(withId(R.id.tag_edit_text)).perform(typeText("expensive, fast, durable"));
            Espresso.onView(withId(R.id.upload_image_button)).perform(click());
            Intents.init();
            Intent resultData = new Intent();
            //click button with text while using the app
            Intents.intending(IntentMatchers.hasAction("while using app")).respondWith(new Instrumentation.ActivityResult(Activity.RESULT_OK, resultData));
            Intents.intending(IntentMatchers.hasAction(MediaStore.ACTION_IMAGE_CAPTURE)).respondWith(new Instrumentation.ActivityResult(Activity.RESULT_OK, resultData));
            Intents.release();
        ...
        private void fillInasus(){
            ...
            com.example.pocketinventory.AddTest.setDate(R.id.date_of_purchase_edit_text, 2020, 9, 15);
            Espresso.onView(withId(R.id.tag_edit_text)).perform(typeText("expensive, fast, durable"));
